...
class AviancaMultiloginCrawler(RequestsBasedAirlineCrawler):
    ...
    def _close_cookie_popup(self, driver):
        try:
            ...
            menu_btn = WebDriverWait(driver, c.HIGH_SLEEP).until(
                ec.element_to_be_clickable(
                    (By.XPATH, '//button[contains(@class, "CookiesBrowserAlert_acceptButtonNO")]')
                )
            )
            menu_btn.click()
    ...
    def login(self, driver):
        try:
            signin_email = '08183922305'
            signin_password = 'Pancakes123@'
            need_login = True

            signin_url = 'https://www.lifemiles.com/fly/find'
            driver.get(signin_url)
            
            try:
                menu_btn_xpath = '//div[contains(@class, "menu-ui-Menu_button")]/span'
                WebDriverWait(driver, c.MEDIUM_SLEEP).until(
                    ec.element_to_be_clickable(
                        (By.XPATH, menu_btn_xpath)
                    )
                )
                need_login = False
            except Exception as e:
            ...
            if need_login:
                ...
                login_btn = WebDriverWait(driver, c.HIGH_SLEEP).until(
                    ec.element_to_be_clickable((By.XPATH, '//a[@class="menu-ui-Menu_login"]'))
                )
                login_btn.click()

                login_option = WebDriverWait(driver, c.HIGH_SLEEP).until(
                    ec.element_to_be_clickable((By.XPATH, '//a[@id="social-Lifemiles"]'))
                )
                login_option.click()
                ...
                username_input.send_keys(signin_email)
                time.sleep(1)
                ...
                driver.find_element(By.ID, "Login-confirm").click()
                time.sleep(1)
...
if __name__ == "__main__":
    ...
    def run(origin, destination, cabin_class, departure_date):
        ...
        crawler = AviancaMultiloginCrawler()
    ...
    run("JFK", "BOG", c.CabinClass.Business, datetime.date(year=2024, month=12, day=6))
