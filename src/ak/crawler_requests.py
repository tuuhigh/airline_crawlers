...
class AlaskaRequestsCrawler(RequestsBasedAirlineCrawler):
    ...
    def get_flights(self,
        ...
        for i, headers_rec in enumerate(headers_recs):
            ...
            json_data.update({
                "origins": [f"{origin.lower()}"],
                "destinations": [f"{destination.lower()}"],
                "dates": [f"{date}"],
            })
    ...
    def _run(
        ...
        **kwargs
        ...
        if not flights_data or not flights_data.get('bestFlights'):
        ...
        flight_search_results = flights_data['slices']
