...
class SingaporeAirRequestCrawler(RequestsBasedAirlineCrawler):
    ...
    def _run(
        ...
        **kwargs
        ...
        sess = request.chrome.Session()
        sess.proxy=self.bota_proxy_str
        # sess.proxies = self.proxy_str
        print(f">>>>>>>>>>>>>>>>>>>>>>>>header1. {sess.headers}")
        for cabin_klass in all_cabins:
            tries = 3
            for cookies_rec in cookies_recs:
            
                tries -= 1
                if tries < 0:
                    raise CrawlerException(f"Tried cookies {tries} times but still got blocked !") 
                ...
                logger.debug(f"> cabin_klass: {cabin_klass} - flight count {len(flights_data)}")
                if not flights_data:
                    if len(all_cabins) > 1:
                        continue
                ...
                for index, flight_search_result in enumerate(flights):
                    ...
                    for fare_name, points_and_cash in points_by_fare_names.items():
                        ...
                        yield flight
                        success = True
            time.sleep(random.uniform(1.13,2.2))
