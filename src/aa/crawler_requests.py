...
class AmericanAirlineRequestsBasedCrawler(RequestsBasedAirlineCrawler):
    AIRLINE = Airline.AmericanAirline

    def get_proxy_attr(self):
        proxy = self.proxy_service.get_rotating_private_proxy()

        proxy_host = proxy.get('ip')  # rotating proxy or host
        proxy_port = proxy.get('port')  # port
    ...
    def _run(
        ...
        **kwargs
        ...
        try:
            proxy_attr = self.get_proxy_attr()
...
if __name__ == "__main__":
    ...
    def run(origin, destination, cabin_class, adults):
        ...
        crawler = AmericanAirlineRequestsBasedCrawler()
    ...
    run("IAD", "CDG", CabinClass.Economy, 1)
